...
export default function ExtractPagesTool() {
  ...
  const handleExtract = async () => {
    if (!file || selectedPages.size === 0) { toast.error("Select at least one page to extract"); return; }
    setLoading(true);
    try {
    ...
    <div className="space-y-6">
      {!file ? (
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${dragOver ? "border-green-500 bg-green-50" : "border-gray-300 hover:border-green-400 hover:bg-green-50/30"}`}
        >
          <Download size={40} className="mx-auto mb-4 text-green-600" />
          <p className="text-xl font-bold mb-1" style={{ color: "var(--text-primary)" }}>Drop PDF to extract pages</p>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="label mb-0">Select pages to EXTRACT <span className="text-green-600">({selectedPages.size} selected)</span></p>
            ...
            <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-2">
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => togglePage(i)}
                  className={`aspect-square rounded-xl font-bold text-sm transition-all flex flex-col items-center justify-center gap-1 border-2 ${selectedPages.has(i) ? "border-green-500 bg-green-100 text-green-700" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>
          ...
          <button onClick={handleExtract} disabled={loading || selectedPages.size === 0}
            className="btn-primary w-full py-4 text-lg flex items-center justify-center gap-3 disabled:opacity-50" style={{ background: "linear-gradient(135deg, #16a34a, #15803d)" }}>
            {loading ? <><Loader2 size={22} className="animate-spin" /> Extracting...</> : <><Download size={22} /> Extract {selectedPages.size} Page{selectedPages.size !== 1 ? "s" : ""}</>}
          </button>
        </div>
      )}
    </div>
